...
for image in file_list:
    ...
    if re.search(pattern1, image):      # this if statement calculates the total leaf area from those images
        ...
        canopy_area, image_with_contours = LAI_c.canopy_calculate_leaf_area(non_green_masked)

        # save the results
        area_data[file_name] = canopy_area
        processed_image = LAI.display_images(original, image_with_contours, canopy_area)
        output_path = os.path.join(result_path, file_name)
        cv2.imwrite(output_path, processed_image)
