...
while t < ts_to_harvest:                 # while time is less than harvest time
    ...
    df_BOS_records = pd.concat([df_BOS_records, dfts], ignore_index=True) # this adds the timestep dataframe to the historical values dataframe
    df_day_TOT = df_BOS_records.groupby(['Day']).sum()
    df_day_AVG = df_BOS_records.groupby(['Day']).mean()
    t += res                          # advance timestep
    i += 1                           # increase matrix index counter
