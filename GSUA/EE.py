    ...
    for item in mec_outputs:   # loop for outputs
        ...
        for item in models:                 # loop for model names
            ...
            if not all(x == 0 for x in X) or not all(y == 0 for y in Y):
                ...
                # print(model_short_name, output_short_name)
                # print(pos2_SEM, neg2_SEM)
                # Add lines from (0, 0) to SEM values
                plt.plot([0, neg2_SEM], [0, max(Y)], color=color, linestyle='--', label='-2SEM Line')
                plt.plot([0, pos2_SEM], [0, max(Y)], color=color, linestyle='--', label='+2SEM Line')


                # plt.plot([0, pos2_SEM], [0, pos2_SEM], color='gray', linestyle='-')
                # plt.plot([neg2_SEM, 0], [neg2_SEM, 0], color='gray', linestyle='-')

                #plot against mu
                # print(X[0], pd.Series(Y[0]))
