            ...
            for item in sobol_tests:
                ...
                if sobol_short_name == 'S2':
                    ...
                    if not S2_small_df[column_name].isna().all():
                        ...
                        plt.errorbar(X, Y, yerr=ciY, fmt = 'o', label= '95% CI', color=color, elinewidth=.5, capsize=2, capthick=.5)
...
#### LOOKS LIKE I LEFT IT AS A STEAMING HOT MESS....
    ...
    #                 print(sobol_short_name)
        ...
        #         ax.scatter(xA, yA)
        #         plt.errorbar(xA, yA, yerr=ciA, fmt='o', color=ami_c, elinewidth=.5, capsize=2, capthick=.5)
        #     if not np.isnan(ciB).all():
        #         ax.scatter(xB, yB)
        #         plt.errorbar(xB, yB, yerr=ciB, fmt='o', color=bos_c, elinewidth=.5, capsize=2, capthick=.5)
        #     if not np.isnan(ciC).all():
        #         ax.scatter(xC, yC)
        #         plt.errorbar(xC, yC, yerr=ciC, fmt='o', color=cav_c,  elinewidth=.5, capsize=2, capthick=.5)
        #     plt.xticks((1, 2, 3, 4, 5), ('TEMP', 'RH', 'CO2', 'PPFD', 'H'))
        #     plt.ylabel('Percent of Output Explained')
        #     plt.xlabel('Equation Inputs')
        #     plt.title(f'{sobol_long_name} Results for {output_short_name}')
