...
def RUN_SIM():     # used to package this version of the MEC as a function callable by other programs
    ...
    ##########################################################
        ...
        df_sims.to_csv(f'{path}/GSUA_CAV_out/data/GSUA_CAV_Simulations.csv') # exports entire final data frame to a CSV
        for output in outputs:      # This loop runs create text files for each /inputoutput of the MEC!
            np.savetxt(f'{path}/GSUA_CAV_out/data/GSUA_CAV_data_{output[0]}.txt', df_sims[[f'{output[0]}']])
...
def RUN_CHART():
    ...
    for item in inputs:        # this allows easy injection of labels into chart elements
        ...
        for item in outputs:
            ...
            plt.savefig(f'{path}/GSUA_CAV_out/figures/CAV {input_short_name} x {output_short_name}.png', bbox_inches='tight') #there are many options for savefig
